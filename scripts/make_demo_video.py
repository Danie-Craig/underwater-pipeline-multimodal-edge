...
def _find_frames(modality: str, cfg: dict, images_override: str | None,
                 stride: int, max_frames: int) -> list[Path]:
    from src.data.loaders import (RGB_DIR, SONAR_DIRS, SONAR_IMG_SUBDIR, NAV_FILE,
                                  ULTRALYTICS_IMG_EXTS, discover_chunks, _sorted_by_timestamp)

    if images_override:
        d = Path(images_override)
        frames = [p for p in d.glob("*")
        ...
        for chunk in discover_chunks(root):
            if modality == "rgb":
                d = chunk / RGB_DIR
                if d.is_dir():
                    frames += [p for p in d.glob("*") if p.suffix.lower() in ULTRALYTICS_IMG_EXTS]
            else:
...
def _edge_note(modality: str) -> str | None:
    ...
    try:
        ...
        return (f"edge: Jetson Orin TRT FP16  "
...
def render_demo(modality: str, cfg: dict, *, max_frames: int, stride: int, fps: float,
    ...
    frames = _find_frames(modality, cfg, images_override, stride, max_frames)
    if not frames:
    ...
    # Warm up once (model load + CUDA init) so the HUD latency reflects the
    # steady state rather than the first-call spike.
    ...
    ema_ms = None
    n_total = len(frames)
    print(f"[demo] {modality}: {n_total} frames -> {out_path.name} (backend={backend}, {fps} fps)")
...
def main() -> None:
    ...
    render_demo(args.modality, cfg, max_frames=args.max_frames, stride=args.stride,
                fps=args.fps, width=args.width, images_override=args.images, backend=args.backend)
